...
class BitMap(object):
    ...
    def __init__(self, maximum=0, bitmap=None, preset=False):
        """
        Create a BitMap
        """
        self.bits = maximum
    ...
    def __getitem__(self, item):
        """
        Return one bit when indexing like an array
        """
    ...
    def __setitem__(self, key, value):
        """
        Sets a bit when indexing like an array
        """
    ...
    @classmethod
    def fromfile(cls, path, maximum=None):
        if maximum is None:
            ...
            bm = cls(maximum)
            file = open(path, 'rb')
            bm.bitmap.fromfile(file, (maximum + 7) // 8)
            file.close()
            return bm
